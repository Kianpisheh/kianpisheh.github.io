import "../App.css";
import Project from "../components/Project";
import Navbar from "../components/Navbar";

import SAHARA from "../assets/SAHARA.svg";
import exHAR from "../assets/exHAR.svg";
import FaceRec from "../assets/FaceRec.svg";
import VidRet from "../assets/VidRet.svg";
import nameLogo from "../assets/Name.svg";
import linkedin from "../assets/linkedin.svg";
import scholar from "../assets/scholar.svg";
import CV from "../assets/CV.svg";

import profile_pic from "../assets/profile.png";
import profile_pic_red from "../assets/profile-red.png";

import { useState } from "react";

export default function Home() {
    const [profileHovered, setProfileHovered] = useState(false);

    const images = [SAHARA, exHAR, FaceRec, VidRet];

    const projects = [
        {
            id: 0,
            title: "SAHARA",
            description:
                "Self-supevised Human Activity Recognition System based on Everyday Audio Events",
        },
        {
            id: 1,
            title: "exHAR",
            description:
                "An Interface for Helping Non-Experts Develop and Debug Knowledge-based Human Activity Recognition Systems",
        },
        {
            id: 2,
            title: "FACE RECOGNITION ASSISTANT",
            description:
                "Face Recognition Assistant for People with Visual Impairments",
        },
        {
            id: 3,
            title: "CONTENT-BASED VIDEO RETRIEVAL",
            description:
                "Content-based Video Retrieval in Traffic Videos using Latent Dirichlet Allocation Topic Model",
        },
    ];

    const currentYear = new Date().getFullYear();

    return (
        <div className="container">
            {/* Navbar */}
            <div className="navbar-container">
                <Navbar logo={nameLogo} />
            </div>

            {/* ABOUT */}
            <div className="about-section">
                <p className="section-title">ABOUT</p>
                <div className="about-content">
                    <p className="about-description">
                        I am a PhD candidate in Computer Science at the
                        <span className="about-emph">
                            {" "}
                            <a
                                className="about-text-link"
                                href="https://web.cs.toronto.edu/"
                            >
                                University of Toronto
                            </a>
                        </span>
                        , working under the supervision of Prof.{" "}
                        <span className="about-emph">
                            <a
                                className="about-text-link"
                                href="https://www.cs.toronto.edu/~khai/"
                            >
                                Khai Truong
                            </a>
                        </span>{" "}
                        in the{" "}
                        <span className="about-emph">
                            <a
                                className="about-text-link"
                                href="https://www.dgp.toronto.edu/"
                            >
                                Dynamic Graphics Project (DGP)
                            </a>
                        </span>{" "}
                        lab. My research lies at the intersection of ubiquitous
                        computing and applied machine learning, focusing on
                        intelligent systems for real-world applications.
                    </p>

                    <div
                        className="about-profile-image"
                        onMouseEnter={() => setProfileHovered(true)}
                        onMouseLeave={() => setProfileHovered(false)}
                    >
                        <img
                            id="profile-pic"
                            src={profileHovered ? profile_pic_red : profile_pic}
                            alt="profile"
                        />

                        <div className="contact-icons-container">
                            <a href="https://www.linkedin.com/in/kian-kianpisheh-0646b283/">
                                <img className="contact-icon" src={linkedin} />
                            </a>
                            <a
                                href="./CV.pdf"
                                target="_blank"
                                rel="noopener noreferrer"
                            >
                                <img className="contact-icon" src={CV} />
                            </a>
                            <a href="https://scholar.google.ca/citations?user=H0dqYeEAAAAJ&hl=en">
                                <img className="contact-icon" src={scholar} />
                            </a>
                        </div>
                    </div>
                </div>
            </div>

            {/* PROJECTS */}
            <div className="projects-section">
                <p className="section-title">PROJECTS AND PUBLICATIONS</p>
                <div className="projects">
                    {projects.map((proj, idx) => (
                        <Project
                            key={idx}
                            image={images[idx]}
                            data={projects[idx]}
                        />
                    ))}
                </div>
            </div>

            {/* FOOTER */}
            <div id="footer">
                <div>{currentYear}. Kian Kianpisheh.</div>
            </div>
        </div>
    );
}

// return (
//     <div className="container">
//         {selectedProj < 0 && (
//             <div className="navbar-container">
//                 <Navbar logo={nameLogo}></Navbar>
//             </div>
//         )}
//         {selectedProj < 0 && (
//             <div className="about-section">
//                 <p className="section-title">ABOUT</p>
//                 <div className="about-content">
//                     <p className="about-description">
//                         I am a PhD candidate in Computer Science at the
//                         <span className="about-emph">
//                             {" "}
//                             <a
//                                 className="about-text-link"
//                                 href="https://web.cs.toronto.edu/"
//                             >
//                                 University of Toronto
//                             </a>
//                         </span>
//                         , working under the supervision of Prof.{" "}
//                         <span className="about-emph">
//                             <a
//                                 className="about-text-link"
//                                 href="https://www.cs.toronto.edu/~khai/"
//                             >
//                                 Khai Truong
//                             </a>
//                         </span>{" "}
//                         in the{" "}
//                         <span className="about-emph">
//                             <a
//                                 className="about-text-link"
//                                 href="https://www.dgp.toronto.edu/"
//                             >
//                                 Dynamic Graphics Project (DGP)
//                             </a>
//                         </span>{" "}
//                         lab. My research lies at the intersection of
//                         ubiquitous computing and applied machine learning ,
//                         focusing on the development of personalized and
//                         customizable smart assistants for real-world
//                         applications. I design and develop intelligent
//                         systems that empower end-users to customize the
//                         system behaviour based on their individual habits,
//                         preferences, and environment.
//                     </p>
//                     <div
//                         className="about-profile-image"
//                         onMouseEnter={() => setProfileHovered(true)}
//                         onMouseLeave={() => setProfileHovered(false)}
//                     >
//                         <img
//                             id="profile-pic"
//                             src={
//                                 profileHovered
//                                     ? profile_pic_red
//                                     : profile_pic
//                             }
//                         ></img>
//                         <div className="contact-icons-container">
//                             <a href="https://www.linkedin.com/in/kian-kianpisheh-0646b283/">
//                                 <img
//                                     className="contact-icon"
//                                     src={linkedin}
//                                 ></img>
//                             </a>
//                             <a
//                                 href="./CV.pdf"
//                                 target="_blank"
//                                 rel="noopener noreferrer"
//                             >
//                                 <img
//                                     className="contact-icon"
//                                     src={CV}
//                                 ></img>
//                             </a>
//                             <a href="https://scholar.google.ca/citations?user=H0dqYeEAAAAJ&hl=en">
//                                 <img
//                                     className="contact-icon"
//                                     src={scholar}
//                                 ></img>
//                             </a>
//                         </div>
//                     </div>
//                 </div>
//             </div>
//         )}
//         {selectedProj < 0 && (
//             <div className="projects-section">
//                 <p className="section-title">PROJECTS AND PUBLICATIONS</p>
//                 <div className="projects">
//                     {projects.map((proj, idx) => (
//                         <Project
//                             key={idx}
//                             image={images[idx]}
//                             data={projects[idx]}
//                             onProjClick={(projId) =>
//                                 setSelectedProj(projId)
//                             }
//                             idx={idx}
//                         ></Project>
//                     ))}
//                 </div>
//             </div>
//         )}

//         {selectedProj >= 0 && (
//             <div className="selected-proj">
//                 <ProjectDetail
//                     key={"proj_detail"}
//                     image={images[selectedProj]}
//                     data={projects[selectedProj]}
//                     onClose={() => setSelectedProj(-1)}
//                 ></ProjectDetail>
//             </div>
//         )}

//         <div id="footer">
//             <div>{currentYear}. Kian Kianpisheh.</div>
//         </div>
//     </div>
// );
